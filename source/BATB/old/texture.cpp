...
static void loadImage128(char* filename)
{
    GLenum gluerr;
    ...
    GLenum format;
    int x, y;
    GLubyte *image = LoadRGBImage(filename, &w, &h, &format);
...
static void loadImage(const char* filename)
{
    GLint width, height;
    GLenum format;
    GLubyte *image = LoadRGBImage(filename, &width, &height, &format);
    ...
    GLubyte* tex = new GLubyte[size];
    int x, y;
    for (y = 0; y < width; y++)
    {
	for (x = 0; x < height; x++)
	{
	  int ri = (y + x*width)*4 + 0;
	  int gi = (y + x*width)*4 + 1;
	  int bi = (y + x*width)*4 + 2;
	  int ai = (y + x*width)*4 + 3;

	  tex[ri] = image[(y + x * width) * nComponents + 0];
	  tex[gi] = image[(y + x * width) * nComponents + 1];
	  tex[bi] = image[(y + x * width) * nComponents + 2];
#if 0
	  if (format == GL_RGBA) {
	    tex[ai] = image[(y + x * width) * nComponents + 3];
	  } else if ((tex[ri] == tex[gi]) && (tex[gi] == tex[bi]) && (tex[bi] == 255)) {
	    tex[ai] = 0;
	  } else {
	    tex[ai] = 255;
	  }
	  ...
	  if (format == GL_RGBA) {
	    tex[ai] = image[(y + x * width) * nComponents + 3];
	  } else if ((tex[ri] > 240) && (tex[gi] > 240) && (tex[bi] > 240)) {
	    tex[ai] = 0;
	  } else {
	    tex[ai] = 255;
	  }
    ...
    GLenum gluerr;
    if ((gluerr = gluBuild2DMipmaps(GL_TEXTURE_2D, 4, width, height, GL_RGBA,
				    GL_UNSIGNED_BYTE, (GLvoid *) (tex))))
    {
	fprintf(stderr, "GLULib%s\n", (char *) gluErrorString(gluerr));
	exit(-1);
