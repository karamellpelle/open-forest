 ...
 */
Fold* make_fold(struct parm *param, int levels,	int stop, Length length)
{
    Fold *p;
...
#if 1
		result->d[i] += 2*sin((double)i);
		//printf("genvalues from scratch, result->d[%d] = %f\n", i, result->d[i]);
#endif
	}
	/*}}}*/
    }
...
void x_update(int count, float scale, float mix,
...
void p_update(int count, float scale, float mix,
	      Strip *a, Strip *b, Strip *c)
{
    int i;
    float w;
...
void t_update(int count, float scale, float mix,
	      Strip *a, Strip *b, Strip *c)
{
    int i;
    float w, we;
...
void v_update(int count, float scale, float mix,
	      Strip *a, Strip *b, Strip *c)
{
    int i;
    float w, we;
...
void hside_update(int count, float scale, float mix, 
		  Strip *a, Strip *b, Strip *c)
{
    int i;
    float w;
