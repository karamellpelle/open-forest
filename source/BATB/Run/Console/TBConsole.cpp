...
TBConsole::TBConsole(BATB* b) : batb( b )
{
    ...
    constexpr const char* path = "static://BATB/Run/console.tb.txt";
    ...
    if ( node.ReadFile( path ) )
    {
        ...
        {
            batb->log << "TBConsole: no TBEditField 'input' defined :(" << std::endl;
        }
        if ( (tb_output_ = GetWidgetByIDAndType<TBEditField>( TBIDC( "output" ) ) ) == nullptr )
        {
            batb->log << "TBConsole: no TBEditField 'output' defined :(" << std::endl;
        }
        {
    ...
    {
        batb->log << "TBConsole: could not read " << path << std::endl;
